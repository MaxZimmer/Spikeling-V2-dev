...
class Spikeling103():
    ...
    def SaveSpikeTraces(self):
        FolderName = QtGui.QFileDialog.getExistingDirectory(self.ui,
                                                      caption='Save Spike data traces on screen',
                                                      directory="./Recordings/Data")
        currentStackedWidget = self.ui.DataAnalysis_Display_StackedWidget.currentWidget()
        ...
        if currentStackedWidget == self.ui.page_103_2_0:
            ...
            df_spikerate = pd.DataFrame(dict_spikerate)
            df_spike = pd.DataFrame(dict_spike)
        df_spikerate.to_csv(FolderName + '/Spikeling_spikerate' + '.csv', index=False)
        df_spike.to_csv(FolderName + '/Spikeling_spike' + '.csv', index=False)
        ...
        if currentStackedWidget == self.ui.page_103_2_1:
            ...
            df_spikerate = pd.DataFrame(dict_spikerate)
            df_spike = pd.DataFrame(dict_spike)
        df_spikerate.to_csv(FolderName + '/AuxNeuron1_spikerate' + '.csv', index=False)
        df_spike.to_csv(FolderName + '/AuxNeuron1_spike' + '.csv', index=False)
        ...
        if currentStackedWidget == self.ui.page_103_2_2:
            ...
            df_spikerate = pd.DataFrame(dict_spikerate)
            df_spike = pd.DataFrame(dict_spike)
        df_spikerate.to_csv(FolderName + '/AuxNeuron2_spikerate' + '.csv', index=False)
        df_spike.to_csv(FolderName + '/AuxNeuron2_spike' + '.csv', index=False)
    ...
    def SaveAverageTraces(self):
        ...
        if currentStackedWidget == self.ui.page_103_3_0:
            ...
            dict_spikerasterplot = {'Spikeling Spike Raster Plot': self.ySpikeLoops0}
            dict_vmaverage = {'Spikeling Vm Average': self.Vm0_mean}
            dict_currentaverage = {'Spikeling Current Average': self.ITotal_mean}
            df_means = pd.DataFrame(dict_spikerateaverage,dict_vmaverage,dict_currentaverage)
            df_rasterplot = pd.DataFrame(dict_spikerasterplot)
        df_means.to_csv(FolderName + '/Spikeling_means' + '.csv', index=False)
        df_rasterplot.to_csv(FolderName + '/Spikeling_rasterplot' + '.csv', index=False)

        if currentStackedWidget == self.ui.page_103_3_1:
            dict_spikerateaverage = {'AuxNeuron1 SpikeRate Average': self.SpikeRate1_mean}
            dict_spikerasterplot = {'AuxNeuron1 Spike Raster Plot': self.ySpikeLoops1}
            dict_vmaverage = {'AuxNeuron1 Vm Average': self.Vm1_mean}
            dict_currentaverage = {'AuxNeuron1 Current Average': self.ISynapse1_mean}
            df_means = pd.DataFrame(dict_spikerateaverage,dict_vmaverage,dict_currentaverage)
            df_rasterplot = pd.DataFrame(dict_spikerasterplot)
        df_means.to_csv(FolderName + '/AuxNeuron1_means' + '.csv', index=False)
        df_rasterplot.to_csv(FolderName + '/AuxNeuron1_rasterplot' + '.csv', index=False)

        if currentStackedWidget == self.ui.page_103_3_2:
            dict_spikerateaverage = {'AuxNeuron2 SpikeRate Average': self.SpikeRate2_mean}
            dict_spikerasterplot = {'AuxNeuron2 Spike Raster Plot': self.ySpikeLoops2}
            dict_vmaverage = {'AuxNeuron2 Vm Average': self.Vm2_mean}
            dict_currentaverage = {'AuxNeuron2 Current Average': self.ISynapse2_mean}
            df_means = pd.DataFrame(dict_spikerateaverage,dict_vmaverage,dict_currentaverage)
            df_rasterplot = pd.DataFrame(dict_spikerasterplot)
        df_means.to_csv(FolderName + '/AuxNeuron2_means' + '.csv', index=False)
        df_rasterplot.to_csv(FolderName + '/AuxNeuron2_rasterplot' + '.csv', index=False)
