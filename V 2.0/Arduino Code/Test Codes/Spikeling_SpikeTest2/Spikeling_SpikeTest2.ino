...
/*              Reading PhotoDiode value and setting PhotoDiode gain for generating input current        */
  ...
  PD_Gain = analogRead(pinPDPot) - bits/2;        // Reads Photodiode Gain potentiometer value and scales it to -2048 to 2048
  PD_Amp = PD_Gain / PDPotScaling;                // Generates an amplification value from the reading and scales it from parameters 

  I_PD = ( PD_Value_Average / PD_Scaling ) * PD_Amp;  // Finally, generates a current input from the photodiode readings, amplified byt the PD_Gain readings
  
  if (PD_Amp > PD_gain_mini){                     // When PD_amp is above the minimum value:
    PD_Amp = PD_Decay * I_PD;                       // Adapts proportionally to I_PD
    if (PD_Amp < PD_gain_mini){                     // If PD_amp becames lower than the minimum value:
      PD_Amp = PD_gain_mini;                          // Then PD_amp remains at the minimum value
    }
  }
